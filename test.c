...
/**
 * @brief Reads exactly @p nbytes, retrying on EINTR/short reads.
 */
static int read_exact(int fd, void *buf, size_t nbytes)
{
	ssize_t n;
	size_t done = 0;
	...
	while (done < nbytes)
	{
		n = read(fd, (char *)buf + done, nbytes - done);
		...
		if (n < 0)
		{
			...
			if (flags & VERBOSE)
				fprintf(stderr, "debug: read_exact: %s\n", strerror(errno));
			return (-1);
		}
		...
		{
			if (flags & VERBOSE)
				fprintf(stderr, "debug: read_exact got EOF after %zu/%zu bytes\n", done, nbytes);
			return (-1);
		}
...
/**
 * @brief Writes exactly @p nbytes, retrying on EINTR/short writes.
 */
static int write_exact(int fd, const void *buf, size_t nbytes)
{
	ssize_t n;
	size_t done = 0;
	...
	while (done < nbytes)
	{
		n = write(fd, (const char *)buf + done, nbytes - done);
		...
		if (n < 0)
		{
			...
			if (flags & VERBOSE)
				fprintf(stderr, "debug: write_exact: %s\n", strerror(errno));
			return (-1);
		}
		...
		{
			if (flags & VERBOSE)
				fprintf(stderr, "debug: write_exact wrote 0 after %zu/%zu bytes\n", done, nbytes);
			return (-1);
		}
...
{
#define WORK_ROUNDS 6
	static volatile int work_sink;
	int c = 0;

	/*
	 * Moderate CPU load: heavy enough to accumulate a few scheduler ticks
	 * even on slow emulators, but short enough to avoid long stalls.
	 */
	for (int r = 0; r < WORK_ROUNDS; r++)
	{
...
/**
 * @brief Two CPU-bound siblings should split CPU roughly 50/50.
 *
 * @returns Zero if passed, non-zero otherwise.
 */
static int sched_test4(void)
{
	const char *path = "sched4.tmp";
	pid_t pid[2];
	long utime[2];
	int fd;

	if ((fd = open(path, O_CREAT | O_RDWR | O_TRUNC, S_IRUSR | S_IWUSR)) < 0)
		return (-1);
	...
	for (int i = 0; i < 2; i++)
	{
		...
		if (pid[i] == 0)
		{
			...
			if ((cfd = open(path, O_WRONLY)) < 0)
				_exit(EXIT_FAILURE);

			for (int k = 0; k < 3; k++)
				work_cpu();
			times(&t);
			utime[i] = (t.tms_utime == 0) ? 1 : t.tms_utime;

			if (lseek(cfd, i * (off_t)sizeof(long), SEEK_SET) < 0)
				_exit(EXIT_FAILURE);
			if (write_exact(cfd, &utime[i], sizeof(long)) != 0)
				_exit(EXIT_FAILURE);
	...
	if (lseek(fd, 0, SEEK_SET) < 0)
		return (-1);
	if (read_exact(fd, utime, sizeof(utime)) != 0)
		return (-1);

	close(fd);
	unlink(path);

	long avg = (utime[0] + utime[1]) / 2;
	if (avg == 0)
		return (-1);
	long diff = (utime[0] > utime[1]) ? (utime[0] - utime[1]) : (utime[1] - utime[0]);

	if (flags & VERBOSE)
		printf("debug: sched4 avg=%ld diff=%ld\n", avg, diff);

	/* Allow 25% deviation from perfect sharing. */
	return (diff > (avg / 4));
}
...
/**
 * @brief Three CPU-bound siblings should split CPU roughly 1/3 each.
 *
 * @returns Zero if passed, non-zero otherwise.
 */
static int sched_test5(void)
{
	const char *path = "sched5.tmp";
	pid_t pid[3];
	long utime[3];
	int fd;
	...
	long avg = (utime[0] + utime[1] + utime[2]) / 3;
	if (avg == 0)
		return (-1);

	for (int i = 0; i < 3; i++)
	{
		long diff = (utime[i] > avg) ? (utime[i] - avg) : (avg - utime[i]);
		if (flags & VERBOSE)
			printf("debug: sched5 child=%d avg=%ld diff=%ld\n", i, avg, diff);
		if (diff > (avg * 3) / 10) /* allow 30% deviation */
			return (-1);
	}
...
{
	/* Be resilient to pipe misuse in tests and keep stdout flushed. */
	...
	for (int i = 1; i < argc; i++)
	{
		...
		else if (!strcmp(argv[i], "sched"))
		{
			...
			printf("  fair 2 procs       [%s]\n",
				   (!sched_test4()) ? "PASSED" : "FAILED");
			printf("  fair 3 procs       [%s]\n",
