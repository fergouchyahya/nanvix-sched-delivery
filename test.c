...
/**
 * @brief Test de priorité stricte.
 * On vérifie que le processus de haute priorité termine avant le bas.
 */
static int sched_test_priority_strict(void)
{
    pid_t pid;

    printf("  [Test] Haute priorite vs Basse priorite...\n");
    ...
    if (pid == 0)
    {
        /* Processus Enfant : Basse priorité (Étudiant) */
        nice(10); // Augmente la valeur nice = baisse la priorité
        work_cpu();
        printf("  [Enfant] Basse priorite termine.\n");
        _exit(EXIT_SUCCESS);
    }
    else
    {
        /* Processus Parent : Haute priorité (Président) */
        nice(-10); // Baisse la valeur nice = augmente la priorité
        work_cpu();
        printf("  [Parent] Haute priorite termine.\n");
        wait(NULL);
    }
...
/**
 * @brief Test Round-Robin au sein d'une même classe.
 */
static int sched_test_rr_fairness(void)
{
    pid_t pids[3];

    printf("  [Test] Round-Robin entre egaux...\n");
    ...
    for (int i = 0; i < 3; i++)
    {
        ...
        if (pids[i] == 0)
        {
            /* Tous ont la même priorité par défaut */
            work_cpu();
            _exit(EXIT_SUCCESS);
        }
    }
...
int main(int argc, char **argv)
{
	...
	for (int i = 1; i < argc; i++)
	{
		...
		else if (!strcmp(argv[i], "sched"))
		{
			...
			printf("  priority strict   [%s]\n",
        		(!sched_test_priority_strict()) ? "PASSED" : "FAILED");
    		printf("  rr fairness       [%s]\n",
        		(!sched_test_rr_fairness()) ? "PASSED" : "FAILED");
		}
